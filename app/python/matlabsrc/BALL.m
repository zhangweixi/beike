...
if  ~isempty(output)
    ...
    if ~isempty(longpass3)
        ...
        output1 = []; k = 1;
        while i <= z
            if find(longpass3(:,1) ~= output(i,1))
               output1(k,:) = output(i,:);
               k = k+1;
            end
            i = i+1;
        ...
        output1 = output; longpass = zeros(1,6);
    end
    shortpass3 = Long_pass(output1,0.003,4,3,100,n); % 判断短传
    if ~isempty(shortpass3)
        [m,~] = size(shortpass3); pass_r_t = []; pass_r_s = [];
        k = 1; j = 1;
        for i = 1:m
    ...
    [m,~] = size(output1);
    while i <= m
        if find(shortpass3(:,1) ~= output1(i,1))
