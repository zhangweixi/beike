...
class Match1Controller extends Controller
{
    ...
    public function upload_match_data(Request $request)
    {
        ...
        $dataType   = $request->input('dataType');//gps,sensor,compass
    ...
    public function handle_data(Request $request)
    {
        ...
        foreach($datas  as $data)
        {
            ...
            $str    .= $this->delete_head($data,$matchData['user_id']);
        }
        ...
        if($type == 'gps')
        {
            ...
            //罗盘数据
            return $this->hand_compass_data($str,$matchData);
     ...
     * 去除头部
     * */
    private function delete_head($str,$userid)
    {
    ...
    public function handle_gps_data($dataSource,$matchData)
    {
        ...
        file_put_contents(public_path('gps-'.$matchData['user_id'].'.json'),\GuzzleHttp\json_encode($matlabData));
        return $matlabData;
    }
    ...
    public function hand_sensor_data($dataSource,$matchData)
    {
        ...
        file_put_contents(public_path('sensor-'.$matchData['user_id'].'.json'),\GuzzleHttp\json_encode($data));
        return $data;
