...
class AnalysisMatchData implements ShouldQueue
{
    ...
    static function call_matlab_calculate($act,$id)
    {
        ...
        mylogger("cd {$filepath} && python run --path={$fpath} --act=match");
        return shell_exec("cd {$filepath} && python run --path={$fpath} --act=match");
    }
    ...
    public function call_matlab_court_action($courtId){
        ...
        $result = self::call_matlab_calculate("court",$courtId);
